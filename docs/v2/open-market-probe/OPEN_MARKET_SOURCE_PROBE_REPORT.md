...
No inventory is reported. The requirement was to read an actual DNB resource, but the official DNB statistical pages returned HTTP 403 during this execution. Therefore it would be misleading to claim that the resource contains (or excludes) AEX, S&P 500, FTSE 100, DAX, Nikkei, TOPIX, or any other index. Each remains `third_party_rights_unresolved` pending direct resource access and source-level provenance.
